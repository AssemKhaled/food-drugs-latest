...
//	Optional<List<Device>> findAllByUserIdInAndDeleteDate(List<Long> userIds , String deleteDate, Pageable pageable);
	...
	@Query(value = "SELECT tc_devices.id,DATEDIFF(tc_devices.end_date,CURRENT_DATE()) as leftDays,tc_devices.name FROM tc_devices"
			+ " INNER JOIN tc_user_device ON tc_user_device.deviceid = tc_devices.id"
			+ " WHERE tc_user_device.userid IN (:userIds ) and tc_devices.delete_date is null",nativeQuery = true)
	public List<DeviceSelectEDIT> getDeviceSelects(@Param("userIds")List<Long> userIds);
	...
	@Query(value = "SELECT id as id ,DATEDIFF(tc_devices.end_date,CURRENT_DATE()) as leftDays ,name as name FROM tc_devices"
			+ " WHERE tc_devices.id IN (:deviceIds ) and tc_devices.delete_date is null",nativeQuery = true)
	public List<DeviceSelectEDIT> getDeviceSelectByIdss(@Param("deviceIds")List<Long> deviceIds);
	...
	@Query(value = "SELECT * FROM tc_devices INNER JOIN tc_user_device ON tc_user_device.deviceid=tc_devices.id "
			+ "where tc_devices.lastupdate>date_sub(now(), interval 0 minute)=false  AND tc_devices.lastupdate<date_sub(now(), interval 3 minute)=false "
			+ " AND tc_user_device.userid IN (:userIds) and tc_devices.delete_date is null and tc_devices.positionid is not null and DATEDIFF(tc_devices.end_date,CURRENT_DATE())>0 ", nativeQuery = true)
	List<Device> newGetNumberOfOnlineDevicesListclient(@Param("userIds")List<Long> userIds);
	...
	@Query(value = "SELECT * FROM tc_devices "
			+ " where tc_devices.lastupdate>date_sub(now(), interval 0 minute)=false  AND tc_devices.lastupdate<date_sub(now(), interval 3 minute)=false "
			+ " AND tc_devices.id IN (:deviceIds) and tc_devices.delete_date is null and tc_devices.positionid is not null and DATEDIFF(tc_devices.end_date,CURRENT_DATE())>0", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM tc_devices INNER JOIN tc_user_device ON tc_user_device.deviceid=tc_devices.id "
			+ "where tc_devices.lastupdate>date_sub(now(), interval 3 minute)=false  AND tc_devices.lastupdate<date_sub(now(), interval 8 minute)=false "
			+ " AND tc_user_device.userid IN (:userIds) and tc_devices.delete_date is null and tc_devices.positionid is not null and DATEDIFF(tc_devices.end_date,CURRENT_DATE())>0", nativeQuery = true)
	List<Device> newGetNumberOfOutOfNetworkDevicesListclient(@Param("userIds")List<Long> userIds);
	...
	@Query(value = "SELECT * FROM tc_devices INNER JOIN tc_user_device ON tc_user_device.deviceid=tc_devices.id "
			+ "where tc_devices.lastupdate>date_sub(now(), interval 8 minute)=false "
			+ " AND tc_user_device.userid IN (:userIds) and tc_devices.delete_date is null and tc_devices.positionid is not null and DATEDIFF(tc_devices.end_date,CURRENT_DATE())>0", nativeQuery = true)
	List<Device> newGetNumberOfOfflineDevicesListclient(@Param("userIds")List<Long> userIds);
	@Query(value = "SELECT tc_devices.positionid FROM tc_devices "
			+ " where  tc_devices.lastupdate>date_sub(now(), interval 8 minute)=false "
			+ " AND tc_devices.id IN (:deviceIds) and tc_devices.delete_date is null and tc_devices.positionid is not null", nativeQuery = true)
	...
	@Query(nativeQuery = true, name = "getDevicesDataMapNoPositionclient")
	List<CustomMapData> getAllDevicesDataMapclient(@Param("userIds")List<Long> userIds);
