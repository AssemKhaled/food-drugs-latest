...
public class DriverServiceImpl extends RestServiceController implements DriverService{
	...
	public DriverServiceImpl(AssistantServiceImpl assistantService, DriverRepository driverRepository, ConnectionToTracer connectionToTracer) {
					...
					if(driver.getName()== null || driver.getUniqueid()== null
						...
						if(!res1.isEmpty() || !res2.isEmpty()) {
							...
							if(driver.getId() == null || driver.getId() == 0) {
								...
								AuditGObject<Driver> newEntity = new AuditGObject<>();
								newEntity.setEntity(driver);
								assistantService.saveOnAuditChanges(id,null,newEntity,"create",driverId);
							...
							if(driverCheck.getDelete_date() == null) {
								...
								if(driver.getName()== null || driver.getUniqueid()== null
									...
									if(!res1.isEmpty() || !res2.isEmpty()) {
										...
										AuditGObject<Driver> oldEntity = new AuditGObject<>();
										AuditGObject<Driver> newEntity = new AuditGObject<>();
										oldEntity.setEntity(driverCheck);
										newEntity.setEntity(driver);
										assistantService.saveOnAuditChanges(id,oldEntity,newEntity,"update",driver.getId());
									    driverRepository.save(driver);
									    drivers.add(driver);
			...
			if(driver != null) {
				if(driver.getDelete_date() == null) {
				 ...
				 if(!checkIfParent(driver , loggedUser) && ! isParent) {
					...
					AuditGObject<Driver> oldEntity = new AuditGObject<>();
					oldEntity.setEntity(driver);
					assistantService.saveOnAuditChanges(userId,oldEntity,null,"delete",driverId);
