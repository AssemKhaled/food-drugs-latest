...
import com.example.food_drugs.entity.DeviceSelectEDIT;
...
public class DeviceServiceImpl extends RestServiceController implements DeviceService {
	...
	public DeviceServiceImpl(ConnectionToTracer connectionToTracer, Utilities utilities, AssistantServiceImpl assistantService) {
...
//					device2.setMiddle_letter(device.getMiddle_letter());
//					device2.setRight_letter(device.getRight_letter());
...
//					device2.setAttributeDevice(device.getAttributeDevice());
//					device2.setDate_type(device.getDate_type());
//					device2.setStoringCategory(device.getStoringCategory());
//					device2.setAttributes(device.getAttributes());
					deviceRepository.save(device);
					AuditGObject<Device> newEntity = new AuditGObject<>();
					newEntity.setEntity(device);
					assistantService.saveOnAuditChanges(userId,null,newEntity,"create",device.getId());
			    	List<Device> devices = null;
	...
	public ResponseEntity<?> editDevice(String TOKEN,Device device, Long userId) {
		...
		if(device.getId() == null || device.getName() == null ||device.getName().equals("") 
	        ...
	        if(duplictionList.size()>0)
		    {
				...
				final Device device1 = oldDevice;
...
//				}
				AuditGObject<Device> oldEntity = new AuditGObject<>();
				AuditGObject<Device> newEntity = new AuditGObject<>();
				oldEntity.setEntity(device1);
				newEntity.setEntity(device);
				device.setId(res.getId());
				logger.info(device.getId());
				assistantService.saveOnAuditChanges(userId,oldEntity,newEntity,"update",device.getId());
				deviceRepository.save(device);
		...
		if(!loggedUser.getAccountType().equals(1)) {
		 ...
		 else
		 {
	         ...
	         device.setUniqueid(uniqueId+"/D");
				 AuditGObject<Device> oldEntity = new AuditGObject<>();
				 oldEntity.setEntity(device);
				 assistantService.saveOnAuditChanges(userId,oldEntity,null,"delete",deviceId);
			 deviceRepository.save(device);
	...
	public ResponseEntity<?> findDeviceById(String TOKEN,Long deviceId, Long userId) {
		...
		if (device == null) {
			...
			else
			{
			   ...
			   JSONObject jo=new JSONObject(device.getAttributes());
			   String s=jo.getString("storingCategory");
			   device.setStoringCategory(s);
				devices.add(device);
				getObjectResponse = new GetObjectResponse(HttpStatus.OK.value(), "success",devices);
				logger.info("************************ getDeviceById ENDED ***************************");
				return ResponseEntity.ok().body(getObjectResponse);
	...
	public  ResponseEntity<?> getDeviceSelectGroup(String TOKEN,Long loggedUserId,Long userId,Long groupId) {
		...
		List<DeviceSelectEDIT> devices = new ArrayList<>();
		if(TOKEN.equals("")) {
			 getObjectResponse = new GetObjectResponse(HttpStatus.BAD_REQUEST.value(), "TOKEN id is required",devices);
			 return  ResponseEntity.badRequest().body(getObjectResponse);
		}
	    		...
	    		if(loggedUser.getDeleteDate() == null) {
	    			if(loggedUser.getAccountType().equals(4)) {	    				
	    				List<Long> deviceIds = userClientDeviceRepository.getDevicesIds(loggedUserId);
		   				 ...
		   				 if(deviceIds.size()>0) {
		   						devices = deviceRepository.getDeviceSelectByIdss(deviceIds);
	    ...
	    if(userId != 0) {
			...
			devices = deviceRepository.getDeviceSelects(usersIds);
	...
	public  ResponseEntity<?> getDeviceSelect(String TOKEN,Long loggedUserId,Long userId) {
		...
		logger.info("************************ getDeviceSelect STARTED ***************************");
		List<DeviceSelectEDIT> devices = new ArrayList<DeviceSelectEDIT>();
		if(TOKEN.equals("")) {
			 getObjectResponse = new GetObjectResponse(HttpStatus.BAD_REQUEST.value(), "TOKEN id is required",devices);
			 return  ResponseEntity.badRequest().body(getObjectResponse);
		}
			...
			if(deviceIds.size() >0) {
			    ...
			    if(size > 0) {
					for(int i=0;i<allDevicesLiveData.size();i++) {
						...
						if (allDevicesLiveData.get(i).getLeftDays()==null||allDevicesLiveData.get(i).getLeftDays() < 0){
							allDevicesLiveData.get(i).setLeftDays(Long.valueOf(0));
						}
...
//					List<CustomMapData> allDevicesPositionOnline= mongoPositionRepo.getOnlineList(positionIds);
			...
			List<CustomMapData> allDevicesNoPosition = deviceRepository.getAllDevicesDataMapclient(usersIds);
...
//			List<String> positionIdsOffline =  deviceRepository.getNumberOfOfflineDevicesList(usersIds);
			List<Device> positionIdsOffline = deviceRepository.newGetNumberOfOfflineDevicesListclient(usersIds);
//			List<String> positionIdsOutOfNetwork =  deviceRepository.getNumberOfOutOfNetworkDevicesList(usersIds);
			List<Device> positionIdsOutOfNetwork =  deviceRepository.newGetNumberOfOutOfNetworkDevicesListclient(usersIds);
//			List<String> positionIdsOnline =  deviceRepository.getNumberOfOnlineDevicesList(usersIds);
			List<Device> positionIdsOnline =  deviceRepository.newGetNumberOfOnlineDevicesListclient(usersIds);
