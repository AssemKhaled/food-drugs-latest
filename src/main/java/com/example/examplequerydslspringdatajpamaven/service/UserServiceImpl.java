...
public class UserServiceImpl extends RestServiceController implements IUserService {
	...
	public UserServiceImpl(UserRepository userRepository, AssistantServiceImpl assistantService, UserRoleRepository roleRepository, UserRoleService userRoleService, TokenSecurity tokenSecurity) {
	...
	public ResponseEntity<?> editUser(String TOKEN,User user,Long userId) {
		...
		if(loggedUser.getAccountType() == 4) {
	    	...
	    	return updateUser(loggedParent.getId(),user);
		...
		if(loggedUser.getAccountType() == 3) {
			...
			String password = oldUser.getPassword();
			user.setPassword(password);
	    	return updateUser(loggedUser.getId(),user);
		...
		if(loggedUser.getAccountType() == 2) {
			...
			if(oldUser.getAccountType() == 4) {
				...
				if(isParent) {
					String password = oldUser.getPassword();
					user.setPassword(password);
			    	return updateUser(parentUsers.getLong("clientId"),user);
				}else {
					getObjectResponse = new GetObjectResponse(HttpStatus.NOT_FOUND.value(), "you are not the parent admin of this user ",null);
			    	return ResponseEntity.status(404).body(getObjectResponse);
				}
		...
		if(loggedUser.getAccountType() == 1) {
			...
			if(oldUser.getAccountType() == 3) {
				...
				if(isParent) {
					String password = oldUser.getPassword();
					user.setPassword(password);
			    	return updateUser(parentUsers.getLong("vendorId"),user);
				}else {
	...
	public ResponseEntity<?> saveUser(Long parentId, User user) {
		...
		else
		{
			...
			AuditGObject<User> newEntity = new AuditGObject<>();
			newEntity.setEntity(user);
			assistantService.saveOnAuditChanges(parentId,null,newEntity,"create",user.getId());
	...
	@Override
	public ResponseEntity<?> updateUser(Long parentId, User user) {
		// TODO Auto-generated method stub
		Set<User> userCreater=new HashSet<>() ;
		userCreater.add(findById(parentId));
		user.setUsersOfUser(userCreater);
		...
		else
		{
			user.setIs_deleted(null);
			user.setDeleteDate(null);
			AuditGObject<User> oldEntity = new AuditGObject<>();
			AuditGObject<User> newEntity = new AuditGObject<>();
			oldEntity.setEntity(oldUser);
			newEntity.setEntity(user);
			assistantService.saveOnAuditChanges(parentId,oldEntity,newEntity,"update",user.getId());
			userRepository.save(user);
			getObjectResponse = new GetObjectResponse(HttpStatus.OK.value(), "success", null);
			logger.info("************************createUser ENDED ***************************");
			return ResponseEntity.ok().body(getObjectResponse);
				...
				boolean isParent = false;
				 ...
				 if(!isParent) {
	    			...
	    			String date = year +"-"+ month +"-"+ day;
	   				deletedUser.setDeleteDate(date);
					AuditGObject<User> oldEntity = new AuditGObject<>();
					oldEntity.setEntity(deletedUser);
					assistantService.saveOnAuditChanges(userId,oldEntity,null,"delete",deleteUserId);
	   				    ...
	   				    //TokenSecurity.getInstance().removeActiveUserById(deleteUserId);
		   				  ...
		   				  if(!children.isEmpty()) {
		   					  for(User object : children ) {
								  ...
								  object.setDeleteDate(date);
								  AuditGObject<User> oldEntitys = new AuditGObject<>();
								  oldEntitys.setEntity(object);
								  assistantService.saveOnAuditChanges(userId,oldEntitys,null,"delete",object.getId());
				...
				boolean isParent = false;
				 ...
				 if(!isParent) {
					 getObjectResponse = new GetObjectResponse(HttpStatus.BAD_REQUEST.value(), "you are not allowed to active this user",null);
	   				...
	   				deletedUser.setDeleteDate(null);
	   				userRepository.save(deletedUser);	   				
	    			getObjectResponse = new GetObjectResponse(HttpStatus.OK.value(),"success",null);
	    			logger.info("************************activeUser ENDED ***************************");
	    			return ResponseEntity.ok().body(getObjectResponse);
	...
	public  ResponseEntity<?> getUserSelectWithChild(String TOKEN,Long userId) {
		...
		List<UserSelect> users = new ArrayList<>();
		if(TOKEN.equals("")) {
			 getObjectResponse = new GetObjectResponse(HttpStatus.BAD_REQUEST.value(), "TOKEN id is required",users);
			 return  ResponseEntity.badRequest().body(getObjectResponse);
		}
